...
const Agenda = () => {
  ...
  // NAYA STATE: Buffer time ke liye
  ...
  const playLoudAlarm = () => {
    try {
      const alarm = new Audio('/sounds/buzzer.mp3');
      alarm.volume = 1.0; 
      alarm.play().catch(e => {
  ...
  useEffect(() => {
    let timer;
    // Condition me !isTransitioning add kiya taaki buffer ke time countDown na chale
    if (workoutMode === 'active' && !isPaused && !isTransitioning) {
      
      if (timeLeft > 0) {
        timer = setInterval(() => setTimeLeft((prev) => prev - 1), 1000);
      } else if (timeLeft === 0) {
        
        // Timer khatam. Transition phase shuru karo aur alarm bajao!
        setIsTransitioning(true);
        playLoudAlarm(); 
        
        // 3 Seconds ka pause jisme alarm bajega aur user position lega
        setTimeout(() => {
          ...
          // Naya timer set hone ke baad transition khatam karo
          setIsTransitioning(false);
        }, 5000); // 3000ms = 3 seconds ka buffer time
      }
    }
    return () => clearInterval(timer);
  }, [workoutMode, timeLeft, isPaused, isResting, currentExIndex, fullRoutine, isTransitioning]);
  ...
  const startLiveWorkout = () => {
    ...
    setIsTransitioning(false); // Reset transitioning
    ...
    return (
      <div className="agenda-container live-workout-mode">
        ...
        <motion.div className="timer-container" key={`${currentExIndex}-${isResting}`} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
          ...
          {/* UI change: Jab buffer chal raha ho tab SWITCH! dikhega */}
          <div className="time-display" style={{ fontSize: isTransitioning ? '4rem' : '6rem', letterSpacing: isTransitioning ? '4px' : 'normal' }}>
            {isTransitioning ? "SWITCH!" : `${Math.floor(timeLeft / 60)}:${(timeLeft % 60).toString().padStart(2, '0')}`}
          </div>
        ...
        <div className="controls">
          <button className="pause-btn" onClick={() => setIsPaused(!isPaused)} disabled={isTransitioning}>
